---
name: blueprint-codegen-setup
description: Setup inicial do codegen (roda 1x) — gera AGENTS.md router, tipos compartilhados, schema e scaffold.
---
<!-- GERADO por tools/build-codex.py a partir de skills/codegen-setup/SKILL.md.
...
1. **`AGENTS.md` router** — diz ao Codex quais docs ler para cada tipo de tarefa
...
> Conflito entre um tipo do prototipo e `04-domain-model.md` significa que `blueprint-prototype-api` deixou um achado passar. Registre em `docs/prototype/05-findings.md` e resolva no blueprint — nunca no tipo.
...
## Passo 5: Gerar o AGENTS.md Router

Use `docs/templates/agentsmd-template.md` como base se existir. Caso contrario:
...
## Antes de Codar
Leia apenas o necessario. `blueprint-codegen-feature` guia a selecao de docs por tipo de feature.
Sempre leia: `src/contracts/` (tipos) + `{{schema}}` (DB, quando relevante).
```

Salve em `{{projeto-alvo}}/AGENTS.md`.
...
>
> Rode `blueprint-codegen` para ver as entregas do build plan, ou `blueprint-codegen-feature [nome]` para a primeira feature."
