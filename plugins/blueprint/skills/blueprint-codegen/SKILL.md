---
name: blueprint-codegen
description: Orquestrador de codegen. Le build plan e guia execucao na ordem correta.
---
<!-- GERADO por tools/build-codex.py a partir de skills/codegen/SKILL.md.
...
2. Verifique se o AGENTS.md existe no projeto-alvo e se `src/contracts/` existe. Se faltar algum:
   > "Setup inicial nao encontrado. Rode `blueprint-codegen-setup` para gerar o AGENTS.md router e o scaffold com os contratos."
...
> **Itens pendentes:**
> 1. {{item 1}} — use `blueprint-codegen-feature {{nome}}`
> 2. {{item 2}} — use `blueprint-codegen-feature {{nome}}`
> 3. {{item 3}} — use `blueprint-codegen-feature {{nome}}`
>
...
3. Se tudo ok, sugira:
   > "Rode `blueprint-codegen-feature {{nome-da-feature}}` para implementar."
...
>
> **Recomendacoes:**
> 1. Rode `blueprint-codegen-verify` para verificar aderencia ao blueprint
...
`blueprint-codegen-setup` (1x) → `blueprint-codegen` (inicio de sessao) → `blueprint-codegen-feature [nome]` (por feature) → `blueprint-codegen-verify` (periodico)
