...
---
<!-- GERADO por tools/build-codex.py a partir de skills/backend/SKILL.md.
...
> "O prototipo registrou {{N}} achados de risco alto ainda abertos. Um contrato construido sobre lacuna conhecida propaga a lacuna para o schema — e schema com dados nao se corrige com `blueprint-increment`.
> Resolva com `blueprint-increment` ou `blueprint-patch`, rode `blueprint-prototype-api` para regenerar o contrato, e volte."

> **Excecao — modo autonomo.** O `blueprint-pipeline` desarma este portao explicitamente, porque num run sem usuario nao ha quem resolva o achado entre uma fase e outra, e achado de risco alto e o *resultado esperado* da fase de prototipo, nao uma anomalia. Nesse caso os 15 documentos sao gerados marcando no ponto afetado `<!-- construido sobre lacuna conhecida: {achado} -->`, e o relatorio final do pipeline diz em voz alta quantos documentos nasceram assim. O portao vale integralmente no modo interativo — que e onde ele pode ser cumprido.
...
> - `blueprint-codegen-setup` — AGENTS.md router + shared kernel (tipos, schema, scaffold)
> - `blueprint-codegen` — iniciar geracao de codigo
> - `blueprint-frontend` — blueprint do frontend
