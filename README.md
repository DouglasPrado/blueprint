...
> Those are the templates the repository ships. What `/blueprint:pipeline` actually *fills* is **51** (57 with the prototype). The difference is `docs/shared/MAPPING.md`, which describes the framework rather than your project and ships complete — the other three cross-layer documents are generated by `/blueprint:shared`.
