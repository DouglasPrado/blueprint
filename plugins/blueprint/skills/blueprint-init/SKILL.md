...
---
<!-- GERADO por tools/build-codex.py a partir de skills/init/SKILL.md.
...
Localize os templates do plugin em `${PLUGIN_ROOT}/docs/`. Se a variavel nao resolver, pare e diga ao usuario que o plugin parece nao estar instalado corretamente — em vez de adivinhar caminho.
...
Para cada suite escolhida, copie de `${PLUGIN_ROOT}/docs/{suite}/` para `./docs/{suite}/`.
...
> Depois rode `blueprint` para a analise de cobertura e o roteiro das 6 fases.
>
