...
---
<!-- GERADO por tools/build-codex.py a partir de skills/build/SKILL.md.
     Nao edite aqui: edite a fonte e rode `python3 tools/build-codex.py`. -->

# Build — Loop de Implementacao com Portoes

Implementa as features do build plan em sequencia, cada uma via `blueprint-codegen-feature` num subagente com contexto limpo. **Para na primeira falha real** em vez de acumular deriva.

```
blueprint-build                      # todas as entregas Must, em ordem de dependencia
blueprint-build ENT-001 ENT-002      # entregas especificas
blueprint-build --max 5              # limita a 5 features nesta sessao
```

## Por que este loop para e o `blueprint-pipeline` nao

O `blueprint-pipeline` pula uma fase que falha: um documento ruim nao contamina o proximo. Aqui e o oposto — a feature 1 estabelece abstracoes que a feature 8 herda. Uma feature errada contamina todas as seguintes, e o resultado final e um codigo internamente consistente, todo verde, e arquiteturalmente errado.
...
|---|---|
| `AGENTS.md` existe no diretorio atual | "Rode `blueprint-codegen-setup` primeiro, ou `cd` para o projeto-alvo." **pare** |
| `src/contracts/` existe e tem tipos | "Scaffold ausente ou vazio. Rode `blueprint-codegen-setup`." **pare** |
| Blueprints alcancaveis (caminho em `AGENTS.md`) | "Nao encontrei os blueprints. Verifique o caminho em `AGENTS.md`." **pare** |
...
Quebre cada entrega nas features que `blueprint-codegen-feature` implementa (uma feature = um vertical slice: banco + API + frontend + testes). Se uma entrega listar 4 itens, sao 4 features.
...
> | ... | ... | ... | ... | ja implementada |
>
> **Portoes:** suite verde por feature (1 retry) · `blueprint-codegen-verify` a cada 3 features (minimo 90%)
...
Divergencias classificadas como `doc desatualizado` nao sao erro de codigo — liste-as separadamente na parada, porque a correcao e `blueprint-increment`, nao `blueprint-codegen-feature`.
...
>
> **Para destravar:**
> - Codigo errado → corrija e rode `blueprint-build` de novo (retoma daqui)
> - Doc desatualizado → `blueprint-increment`, depois `blueprint-build`
> - Feature ruim ja commitada → `git revert {{sha}}`, ajuste o blueprint, `blueprint-build`
>
> **Nao implementadas:** {{lista das features restantes}}"

Retomar e sempre `blueprint-build` — ele detecta o que ja existe e continua.
...
- **Deriva arquitetural e detectada tarde.** Uma abstracao ruim na feature 1 so aparece no verify da feature 3. Rode `blueprint-build --max 3` nas primeiras features e revise antes de soltar o loop inteiro.
- **Herda as suposicoes dos documentos.** Se o blueprint foi gerado por `blueprint-pipeline` em modo autonomo, o codigo implementa fielmente coisas possivelmente supostas. Revise `docs/ASSUMPTIONS.md` antes.
