...
---
<!-- GERADO por tools/build-codex.py a partir de skills/prototype-api/SKILL.md.
...
```
blueprint-prototype-api [cliente] [projeto-alvo]
```
...
|---|---|
| Prototipo construido no projeto-alvo | "Rode `blueprint-prototype-build` primeiro." **pare** |
| `src/mocks/handlers/` existe e tem handlers | "Nao encontrei a camada de mock. O contrato e extraido dela." **pare** |
| Portoes de `blueprint-prototype-build` passaram | Avise: contrato extraido de prototipo incompleto nasce incompleto |
...
| **Chamadas feitas** | **`src/api/`** — a camada de dados que `blueprint-prototype-build` torna obrigatoria | Quem chama, de qual tela, com que frequencia |
...
- Matriz de estados por tela. Os **quatro obrigatorios** — carregando, vazio, erro, sucesso — precisam estar preenchidos em toda tela que busca dados; celula vazia num deles e tela incompleta e volta para `blueprint-prototype-build`. **Parcial** e **sem permissao** sao condicionais: `n/a` e resposta valida quando a tela nao os admite.
...
Feche com o **encaminhamento**: cada achado com destino (`blueprint-increment` ou `blueprint-patch`) e status.
...
Contrato construido sobre lacuna conhecida propaga a lacuna para o schema — e schema com dados nao se corrige com `blueprint-increment`.
...
> 1. {{achado}} → {{destino}}
>
> Resolva com `blueprint-increment` (local) ou `blueprint-patch` (global) e rode `blueprint-prototype-api` de novo para regenerar o contrato."
...
>
> **Proximo passo:** `blueprint-backend` — agora com `docs/prototype/03-api-requirements.md` como **fonte primaria do contrato**, e o blueprint tecnico como fonte do dominio."
