...
> 1. Passar o caminho: `/blueprint:blueprint docs/prd.md`
...
```
1. /blueprint:blueprint-foundation    → 00-context, 01-vision, 02-principles, 03-requirements
2. /blueprint:blueprint-domain        → 04-domain-model, 05-data-model, 09-state-models
3. /blueprint:blueprint-architecture  → 06-system-architecture, 10-architecture_decisions
4. /blueprint:blueprint-flows         → 07-critical_flows, 08-use_cases
5. /blueprint:blueprint-quality       → 12-testing, 13-security, 14-scalability, 15-observability
6. /blueprint:blueprint-plan          → 11-build_plan, 16-evolution
```
...
> "PRD salvo e analisado. Rode `/blueprint:blueprint-foundation` para comecar pela Fundacao (contexto, visao, principios e requisitos)."

## Depois do Blueprint

| Skill | Para que |
|-------|----------|
| `/blueprint:backend` | Especificacao de implementacao backend (15 docs) |
| `/blueprint:frontend` | Arquitetura frontend multi-client |
| `/blueprint:increment` | Adicionar feature ou corrigir sem reescrever |
| `/blueprint:patch` | Propagar mudanca global (renomear entidade, trocar tecnologia) |
| `/blueprint:specs` | Gerar backlog integral de tasks |
| `/blueprint:codegen` | Gerar codigo a partir dos blueprints |
