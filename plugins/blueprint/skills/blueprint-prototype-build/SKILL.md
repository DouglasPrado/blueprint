---
name: blueprint-prototype-build
...
---
<!-- GERADO por tools/build-codex.py a partir de skills/prototype-build/SKILL.md.
...
```
blueprint-prototype-build [cliente] [projeto-alvo]

blueprint-prototype-build web ../meu-saas/
blueprint-prototype-build web ../meu-saas/ --telas OrdersPage,OrderDetail   # subconjunto
```
...
|---|---|
| `docs/prototype/01-screens.md` sem `{{placeholders}}` | "Rode `blueprint-prototype` primeiro." **pare** |
| `docs/prototype/02-mock-data.md` sem `{{placeholders}}` | "Rode `blueprint-prototype` primeiro." **pare** |
| `docs/frontend/shared/03-design-system.md` preenchido | "Rode `blueprint-frontend-design-system` primeiro." **pare** |
...
1. **Nenhum componente chama `fetch` direto.** Toda chamada HTTP vive em `src/api/{recurso}.ts` e chega ao componente por hook. Esta e a regra que torna a fase seguinte possivel: `blueprint-prototype-api` monta o inventario "chamadas feitas" lendo **este diretorio**. Chamada espalhada por componente nao e inventariavel, e sem o inventario o cruzamento que da valor a fase nao acontece.
2. **Os tipos das entidades saem de `04-domain-model.md`, com os nomes do glossario.** Eles viram `src/contracts/` no `blueprint-codegen-setup` — divergir aqui custa um rename depois.
...
>
> Rode `blueprint-prototype-api {client} {projeto-alvo}` para extrair o contrato que o backend precisa implementar."
