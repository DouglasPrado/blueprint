...
---
<!-- GERADO por tools/build-codex.py a partir de skills/patch/SKILL.md.
...
Para adicionar uma feature ou corrigir um dado em um blueprint so, use `blueprint-increment`.
...
docs/prototype/*.md           # se a fase de prototipo rodou
docs/shared/*.md
...
|------|---------|--------------|
| Renomear entidade | `blueprint-patch` "Booking → Appointment" | domain, data, services, repositories, components, state, hooks |
| Atualizar endpoint | `blueprint-patch` "/api/users → /api/v2/users" | api-contracts, controllers, data-layer, api-dependencies |
| Mudar tecnologia | `blueprint-patch` "Zustand → Jotai" | frontend (state, vision, cicd), blueprint (decisions) |
| Atualizar versao | `blueprint-patch` "Next.js 16 → Next.js 17" | frontend (vision), blueprint (decisions) |
| Renomear componente | `blueprint-patch` "UserCard → ProfileCard" | frontend (components, flows, tests, copies) |
| Renomear feature | `blueprint-patch` "features/auth/ → features/identity/" | frontend (structure, components), backend (structure) |
